...
2. Inside the `tracts_prediction` function, fit three more multiple linear regression models: one using `population` and `income` as predictors, one using `minority` and `income`, and one using all three predictors. Also, modify the function so that `valid_predictions` contains the predictions from all four models.

3. Re-run the chunk with the `tracts_prediction` function, then re-run the chunk creating `mapped_predictions_df`. Produce a calibration plot for the other 3 models.
