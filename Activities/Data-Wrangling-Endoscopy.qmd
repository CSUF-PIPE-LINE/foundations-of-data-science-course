...
* People who spent more than 360 minutes in the postoperative care area following the procedure (PACU_LOS)
...
Using the `endo2` dataset (start over with `endo2` once you finish each problem):
...
If we have a factor variable, the "smallest" value is the first level and the "largest" value is the last level. If the categorical variable is a chracter (string) instead, it will sort in alphabetical order.
...
2. Find the procedure times (`TCL`) for the 5 patients who stayed the longest in the post-operative area (`PACU_LOS`).
...
Questions 3 and 4 require multiple steps to solve and will require careful thought about what you need to do and in what order. I highly recommend talking the procedure through with a partner before trying to code anything, and then talking again if you don't get out what you think you should. 
...
4. Create a two-way table comparing the proportion of patients who spent over 1 hour in the post-operative care area (PACU) among patients with liver disease and patients without liver disease. Your table should include informative category names and not display information for patients whose liver disease status is unknown.