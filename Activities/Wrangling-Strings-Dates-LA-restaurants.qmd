...
#violations <- readr::read_csv(here::here("violations.csv"))
```
...
str_extract(facilities_10_desc, "\\(.+\\)")
```
...
We will use the `mutate` function to seven new variables to the dataset: one to answer each question.
