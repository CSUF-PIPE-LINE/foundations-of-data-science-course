...
bayeslm <- stan_glm(
  Price ~ SqFt + Beds + Baths,
  data = SoCalRent_train,
  prior = normal(location = c(2, 200, 200), scale = c(2, 100, 100), autoscale = TRUE),
...
SoCalRent_test_augmented <- SoCalRent_test |>
  mutate(
    .fitted_low = pred_interval[,1],
    .fitted_high = pred_interval[,2]
  ) 

SoCalRent_test_augmented |>
  select(
    Price,
    .fitted_low,
    .fitted_high,
...
SoCalRent_test2 <- SoCalRent_test_augmented |>
  arrange(Price) |>
  mutate(row = 1:nrow(SoCalRent_test_augmented),
         contains = (.fitted_low < Price & .fitted_high > Price))

ggplot(
  data = SoCalRent_test2,
  aes(x = Price, y = Price)
) +
#  geom_point() +
  geom_segment(
  ...
  labs(
    x = "Actual Rent Price",
    y = "90% Posterior Predictive Interval for Price"
  )
```