...
violations2 <- violations |>
  pivot_wider(
    id_cols = SERIAL_NUMBER,
    names_from = VIOLATION_CODE,
    values_from = POINTS,
    values_fn = sum,
    values_fill = 0
  )
...
inspections2 <- inspections |>
  ...
  arrange(date) |>
  mutate( 
    prev_inspection = lag(SERIAL_NUMBER)
  ) |>
  drop_na(prev_inspection)
...
inspections_joined <- inspections2 |>
  inner_join(
    violations2 ,
    by = join_by(prev_inspection == SERIAL_NUMBER)
  )
...
inspect_split <- inspections_joined |>
  group_initial_split(
    prop = 0.80,
    group = RECORD_ID
  )
...
inspect_train2 <- inspect_train |>
  mutate(
    risk_level = stringr::str_extract(PE_DESCRIPTION, "LOW|MODERATE|HIGH")
  ) |>
  select(
    RECORD_ID,
    PROGRAM_NAME,
...
lm_inspect <- lm(
  SCORE ~ risk_level + SERVICE_DESCRIPTION + F044 + F033 + F007,
  data = inspect_train2
)
