...
- To move variables to the "front" (left side) of the dataset, use `relocate`
...
- Other arguments to `relocate` may allow us to group variables together
...
- For advanced use: Many helper functions can identify multiple variables at once
...
- To subset to specific numbered rows, use `slice`
...
alz |>
  ...
  mutate(row_number = seq(1, n())) |>
  # put row number in the leftmost column
  relocate(row_number) |>
  slice(39:43, 48)
```
...
- To subset to rows that meet a specific condition, use `filter`

- Arguments should be one or more expressions that can evaluate to `TRUE` or `FALSE`

## Boolean Expressions in R

- Equations and inequalities work with either numerical or categorical variables:
...
- Inequalities (usually) only work with numerical variables
...
## Sorting and Selecting Rows in One Line
...
## Sorting and Selecting within Groups

```{r}
#| label: oldest 2 in each group
#| code-line-numbers: "2"
alz |>
  slice_max(age, n = 2, by = diagnosis) |>
  select(subject_ID, diagnosis, age)
```

```{r}
#| label: youngest 2 in each group
#| code-line-numbers: "2"
alz |>
  slice_min(age, n = 2, by = diagnosis) |>
  select(subject_ID, diagnosis, age)
```
...
alz |>
  ...
  select(subject_ID, mmse, cognition) |>
  head(5)
```
...
- R creates two new vectors: one of values assuming everything is `TRUE` and one assuming everything is `FALSE`

    - number of elements = number of rows in the data frame
    
    - individual values will be repeated *n* times
  
- R then creates a third vector and puts in it the value from the `TRUE` or `FALSE` vector as appropriate for each element
...
- Use `if_else` if there are only 2 possible outcomes

- Use `case_when` if there are more than 2 possible outcomes
